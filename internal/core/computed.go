...
type AdvancedPitchingStats struct {
	...
	FIPMinus        *int        `json:"fip_minus,omitempty"` // FIP- / xFIP- (FanGraphs style: lower is better, 100 = league avg), if you want.
	XFIPMinus       *int        `json:"xfip_minus,omitempty"`
	WAR             *float64    `json:"war,omitempty"`     // Total WAR from chosen provider (FG, BR, etc.).
...
type PlayerWARSummary struct {
	...
	TeamID                       *TeamID     `json:"team_id,omitempty"`
	Context                      StatContext `json:"context"`
	WAR                          float64     `json:"war"`                    // Total WAR (position-player + pitcher if two-way).
	...
	ReplacementRuns              *float64    `json:"replacement_runs,omitempty"`
	PitchingRunsAboveReplacement *float64    `json:"pitching_rar,omitempty"` // Alternate view for pitchers.
}
...
type PlayerLeverageSummary struct {
	...
	TeamID                 *TeamID     `json:"team_id,omitempty"`
	Context                StatContext `json:"context"`
	AvgLeverageIndex       float64     `json:"avg_li"`             // Average LI of all plate appearances faced (aLI).
	GameEntryLeverageIndex *float64    `json:"gm_li,omitempty"`    // Average LI at the moment the player entered games (gmLI).
