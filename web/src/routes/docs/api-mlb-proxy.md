...
| `GET /v1/mlb/crosswalk/teams` | MLB Stats API + local DB | Crosswalk MLB team IDs to local `team_id`/`franchise_id` for season-aware routing. |
...
- `/v1/mlb/crosswalk/teams` is served by the MLB route layer and local team repository to provide deterministic team navigation links.
